...
BIN="$ROOT/build/src/openpunchclock"
...
cp "$BIN" "$APPDIR/usr/bin/openpunchclock"
cp "$ROOT/packaging/openpunchclock.png" \
   "$APPDIR/usr/share/icons/hicolor/512x512/apps/openpunchclock.png"

cat > "$APPDIR/usr/share/applications/openpunchclock.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=Open Punch Clock
Comment=Pointeuse / feuille de temps open source
Exec=openpunchclock
Icon=openpunchclock
...
OUTPUT="OpenPunchClock-${VERSION_NAME}-x86_64.AppImage"
export OUTPUT
...
"$TOOLS/linuxdeploy" \
    ...
    --plugin qt \
    --output appimage \
    --desktop-file "$APPDIR/usr/share/applications/openpunchclock.desktop" \
    --icon-file "$APPDIR/usr/share/icons/hicolor/512x512/apps/openpunchclock.png"

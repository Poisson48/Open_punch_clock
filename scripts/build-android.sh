...
OUT="${OUT:-$ROOT/openpunchclock-arm64.apk}"
...
"$QT_ANDROID/bin/qt-cmake" \
  ...
  -DPUNCH_VERSION_NAME="$VERSION_NAME" \
  -DPUNCH_VERSION_CODE="$VERSION_CODE"
...
if [ -n "${ANDROID_KEYSTORE_B64:-}" ]; then
  KEYSTORE="$(mktemp -t openpunchclock-keystore.XXXXXX.jks)"
  ...
  if [ ! -f "$KEYSTORE" ]; then
    ...
    keytool -genkeypair -keystore "$KEYSTORE" -alias "$KEYALIAS" \
      ...
      -dname "CN=Open Punch Clock Debug, O=OpenPunchClock, C=FR" >/dev/null
  fi
